...
└─────────┬──────────────────────────────────────────────────┘
          ...
          ▼
     ┌─────────────┐
     │   LLM API   │  (скоринг, генерация, ревью)
     └─────────────┘
```
...
[Появилась сеть]
   ...
   → FastAPI вызывает LLM по нужной рубрике
...
| **LLM API** | Оценку, генерацию, ревью | Хранение состояния |
...
├── .env.example          ← ЕДИНСТВЕННЫЙ источник окружения: compose, backend и клиент
